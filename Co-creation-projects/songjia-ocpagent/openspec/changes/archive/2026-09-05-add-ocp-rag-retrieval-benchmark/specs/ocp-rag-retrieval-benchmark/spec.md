...
- **THEN** it evaluates every fixture case tagged with document version `4.22`
  using only Qdrant points with that document version

#### Scenario: Run a selected document version and cases
- **WHEN** the user supplies a document-version override and an explicit case
  selection
...
4.22 cases. Each case SHALL contain a unique case id, a non-empty question, a
...
ID SHALL be verified to exist in the configured OCP 4.22 corpus with matching
document version before a benchmark run begins.
...
final reranked results using deduplicated stable point IDs. At each cutoff,
precision SHALL equal matching retrieved IDs divided by returned unique IDs,
and recall SHALL equal matching retrieved IDs divided by expected IDs.
...
- **THEN** the case result records retrieved, matched, and missing IDs and
  calculates precision and recall for top-10, top-20, and top-30 from those
  sets
...
- **THEN** that stage records precision and recall of `0.0` at every required
  cutoff without preventing independent stages or later cases from being
  evaluated
...
per-case stage results, and macro and micro precision/recall at top-10,
top-20, and top-30 for every scored stage. The reports SHALL be written to a
configurable output directory and the command SHALL print their paths.

#### Scenario: All selected cases complete
- **WHEN** every selected case is evaluated successfully
- **THEN** both reports include aggregate metrics calculated across all selected
  cases and case-level retrieval diagnostics
...
- **THEN** the benchmark continues independent cases, records the failed case
  and error in both reports, and excludes that failed stage from aggregate
  metrics
