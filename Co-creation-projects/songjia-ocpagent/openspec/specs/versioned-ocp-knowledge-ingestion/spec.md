...
Ingest a selected version of locally collected OpenShift Container Platform
Markdown as provenance-preserving BGE-M3 hybrid vectors in Qdrant.
...
command SHALL default its Qdrant endpoint to `http://localhost:6333` and
collection name to `ocp-documents` while accepting command-line overrides for
document root, endpoint, collection, model, and batch size.
